/*
 * @Author: Delavin.TnT
 * @LastEditors: Delavin.TnT
 * @Date: 2021-07-27 17:36:22
 * @LastEditTime: 2021-09-30 16:42:19
 */
import { TableColumn } from "@/types/tableColumn";
import { createVNode } from 'vue'
import { subGroup } from '../../components'
import router from '@/router'
import { delDomain } from '@/api/strategy/domain'
export const useColumns = (t): TableColumn[] => [
  {
    title: t('strategy.domainName'),
    dataIndex: 'name',
    width: 200
  },
  {
    title: t('strategy.distNum'),
    dataIndex: 'total',
    width: 100
  },
  {
    title: t('strategy.ineffeQu'),
    dataIndex: 'uneffected',
    width: 100,
    customRender: ({ text }) => createVNode('span', { style: { color: text ? '#f8a04c' : 'rgba(0, 0, 0, 0.85)' } }, text)
  },
  {
    title: t('strategy.polAppGroup'),
    dataIndex: 'policyrecord',
    width: 400,
    slots: {
      customRender: 'policyrecord'
    },
    slotsType: 'component',
    slotsFunc: (record, reload) => createVNode(subGroup, { name: record.name, desc: record.desc, policyrecord: record.policyrecord, policy_type: 1, reload })
  },
  {
    title: t('strategy.lastDate'),
    dataIndex: 'updated_at',
    width: 220
  },
  {
    title: t('common.action'),
    dataIndex: 'action',
    width: 100,
    slots: {
      customRender: 'action_icon'
    },
    actions: [
      {
        type: 'icon',
        actionList: [
          {
            text: t('common.edit'),
            func: ({ record }) => {
              router.push({
                name: 'strategy-domain-detail',
                query: {
                  strategy_id: record.id
                }
              })
            }
          },
          {
            type: 'popconfirm',
            text: t('common.delete'),
            func: async ({ record }, callback) => {
              await delDomain({
                id_string: record.id + ''
              })
              callback()
            }
          },
        ]
      },

    ]
  }
]